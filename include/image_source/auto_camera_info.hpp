...
protected:
  virtual void onInit() {
    ...
    subscriber_ = image_transport::ImageTransport(nh).subscribe(
        "image_in", 1, &AutoCameraInfo::publish, this,
  ...
  void publish(const sensor_msgs::ImageConstPtr &image) {
    ...
    const double fx =
        fov_x_ ? cx / std::tan(*fov_x_ / 2.) : std::sqrt(cx * cx + cy * cy) / std::tan(*fov_ / 2.);
    const double fy =
        fov_y_ ? cy / std::tan(*fov_y_ / 2.) : std::sqrt(cx * cx + cy * cy) / std::tan(*fov_ / 2.);
    info->K = {fx, 0., cx, //
               0., fy, cy, //
               0., 0., 1.};
